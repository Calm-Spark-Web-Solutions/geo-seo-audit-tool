...
export default async function AuditExportPage({
  ...
  return (
    <>
      <PageHeader
        eyebrow={
          <Link href={`/visibility-scans/${typedAudit.id}`} className="hover:underline">
            Back to scan
          </Link>
        }
        title="Export PDF"
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base">
            {typedCommunity?.name ?? "Community"} report
          </CardTitle>
          <CardDescription>
        ...
        <CardContent>
          <div className="overflow-hidden rounded-md border border-border bg-muted/40">
            <iframe
              title={`Visibility scan ${typedAudit.id} PDF preview`}
              src={`/api/visibility-scans/${typedAudit.id}/report.pdf`}
