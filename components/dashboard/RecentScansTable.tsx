...
function scoreDotLabel(score: number | null): string {
      ...
      <div className="border-t border-border">
        {rows.length === 0 ? (
          <p className="px-6 py-8 text-center text-sm text-muted-foreground">
            No visibility scans yet. Open a community to run your first one.
          </p>
        ) : (
          rows.map((s) => (
            <Link
              ...
              className="group flex flex-col gap-2 border-t border-border px-4 py-3 transition-colors hover:bg-accent first:border-t-0 sm:flex-row sm:items-center sm:gap-4 sm:px-6"
            >
              <div className="flex min-w-0 items-center gap-3 sm:flex-1">
                <span
                  className={`h-2 w-2 shrink-0 rounded-full ${scoreDotColor(s.total)}`}
                  role="img"
                  aria-label={`Score status: ${scoreDotLabel(s.total)}`}
                />
              ...
              <div className="flex shrink-0 flex-wrap items-center gap-3 pl-5 sm:gap-4 sm:pl-0">
